...
public:
	RawTcpServer(asio::io_service& io_service, std::vector<std::shared_ptr<asio::io_service>> ioservices)
	...
	{
		strand_.post(std::bind(&RawTcpServer::__Send, this->shared_from_this(), seq, data, pfn));
	}
	...
	{
		strand_.post(std::bind(&RawTcpServer::__SendV, this->shared_from_this(), seq, data, pfn));
	}
	auto BlockRecv(uint32_t seq, bool b) -> void
	{
		strand_.post(std::bind(&RawTcpServer::__BlockRecv, this->shared_from_this(), seq, b));
	}
	auto Close(uint32_t seq) -> void
	{
		strand_.post(std::bind(&RawTcpServer::__Close, this->shared_from_this(), seq));
	}
