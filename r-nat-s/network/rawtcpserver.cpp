#include "stdafx.h"
#include "rawtcpserver.hpp"

auto RawTcpServer::Start(std::vector<asio::ip::tcp::endpoint> eps) -> bool
{
	// init acceptor
	for (auto&& ep : eps)
	{
...
auto RawTcpServer::__StartNextConnection(std::shared_ptr<asio::ip::tcp::acceptor> acceptor) -> void
{
	...
	acceptor->async_accept(conn->Socket(),
		std::bind(&RawTcpServer::__OnAccept, this->shared_from_this(), seq, conn, acceptor, std::placeholders::_1));
}

auto RawTcpServer::__OnAccept(uint32_t seq, std::shared_ptr<RawServerTCPConnection> conn, 
...
auto RawTcpServer::__GetNextSeq() -> uint32_t
{
	uint32_t seq;
	do
	{
...
auto RawTcpServer::GetPort() -> uint16_t
{
	return port_;
}

auto RawTcpServer::__Send(uint32_t seq, std::shared_ptr<asio::streambuf> data, std::function<void(void)> pfn) -> void
{
...
auto RawTcpServer::__BlockRecv(uint32_t seq, bool b) -> void
{
	auto&& iter = conns_.find(seq);
	if (iter != conns_.end())
	{
...
auto RawTcpServer::__Close(uint32_t seq) -> void
{
	auto&& iter = conns_.find(seq);
	if (iter != conns_.end())
	{
